...
import watertap.examples.flowsheets.high_pressure_RO.hpro_only.flowsheet as high_pressure_RO
...
def add_scaling_surrogate(m, surrogate_type="polynomial"):
    ...
    if surrogate_type == "polynomial":
        ...
        gypsum_scaling_index_surrogate = (
            polynomial_regression.PolynomialRegression.pickle_load(dir_path + file_name)
        )
    elif surrogate_type == "rbf":
        file_name = m.case + "_gypsum_surrogate_interpolating.pickle"
        gypsum_scaling_index_surrogate = (
            radial_basis_function.RadialBasisFunctions.pickle_load(dir_path + file_name)
        )

    m.fs.gypsum_scaling_index = Var(
        initialize=1, bounds=(0, None), units=pyunits.dimensionless
    )

    m.fs.product_recovery_indexed = Var(
        [0], initialize=0.73, bounds=(0, 1), units=pyunits.dimensionless
    )
    ...
    m.fs.eq_gypsum_scaling_index = Constraint(
        expr=(
            m.fs.gypsum_scaling_index
            == gypsum_scaling_index_surrogate["model"].generate_expression(
                [m.fs.product_recovery_indexed[0]]
            )
        )
    )
